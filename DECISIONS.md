...
## D21. Phase 3: hard documents, on the phone (2026-09-25)

- **Scanned pages** are read with Tesseract.js (Apache-2.0; engine and English data from jsDelivr once, then cached). Each page is drawn at about 180 dpi; words under 55% confidence are dropped, and a page with fewer than 12 confident words is treated as a picture. The words go back into the layout engine as positioned text, so scans get the same column, header and page-number handling. Runs by itself after import, one document at a time, saving each page as it is read so it resumes after the app closes; failures show in the library row.
- **Measured on the PC:** a scanned copy of the Stark paper's two-column page 2 (rendered to JPEG, wrapped in an image-only PDF) was read in about 14 s; 89% of its words recovered, 93% of what was read correct, both columns found and read in order. On the phone, expect several seconds a page.
- **Figures:** images are found from PDF.js's drawing operations (2.1 s for the 126-page CPPS report on the PC). An image covering 4 to 85% of a page becomes a figure card; text lying on it (map labels, chart values) goes with the figure, not the prose. Charts drawn as vector graphics leave bare numbers behind; a paragraph that is at least 70% numbers becomes "Numbers", shown but not read.
